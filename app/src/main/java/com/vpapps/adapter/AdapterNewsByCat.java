...
public class AdapterNewsByCat extends RecyclerView.Adapter {
    ...
    public AdapterNewsByCat(Context context, ArrayList<ItemNews> arrayList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == VIEW_ITEM) {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_newsbycat, parent, false);
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            if(isHeader(holder.getAdapterPosition())) {
                imageurl = methods.getImageThumbSize(arrayList.get(holder.getAdapterPosition()).getImageThumb(), "header");
            } else {
                imageurl = methods.getImageThumbSize(arrayList.get(holder.getAdapterPosition()).getImageThumb(), "notheader");
            }
            Picasso.get()
                    .load(imageurl)
    ...
    public boolean isHeader(int position) {
        return (isProgressPos(position) || (position + 3) % 3 == 0);
    }
